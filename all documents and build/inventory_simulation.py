...
PURPOSE
-------
The thesis (Chapter 5) reports a ~21% annual cost reduction for the
ML-Empirical-Quantile policy over the Classical baseline. That figure is
NOT simulated: the achieved service level of each policy is *assigned*
analytically (Gaussian -> ~83%, Quantile -> ~95% "by calibration") and then
plugged into  SC = (target_SL - achieved_SL) * D * unit_cost * m.
Because the service levels are inputs rather than measured outcomes, the
quantile policy wins by construction.
...
def main():
    ...
    out = "data/processed/simulation_results.csv"
