"""Generate Chapter 6 - Discussion (revised to align with the supervisor's
...
add_para(doc,
    ...
    "policies achieve full service on the test window. When the achieved service "
    "level of each policy is measured by simulating the policy forward day-by-day over "
    "the 56-day test window — rather than inferred from a Gaussian projection onto the "
...
add_para(doc,
    ...
    "constrained resources. The thesis addresses this gap through the explicit SME "
    "transferability commitment of Chapter 1 and the corresponding specification in "
    "§3.11.")
add_para(doc,
    ...
    "Microsoft Power BI Desktop free tier. No commercial software licence or cloud-"
...
add_heading(doc, "6.3.4 Gap 4 — Responsible AI and uncertainty integration", 2)
add_para(doc,
    "Gap 4 concerns the limited treatment of forecast uncertainty in the safety-stock "
    "calibration step (the Gaussian formula assumes point-forecast inputs and parametric "
    "normality) and the under-addressed responsible-AI dimension of retail forecasting.")
add_para(doc,
    "The thesis addresses Gap 4 in two ways. First, the LightGBM quantile-regression "
    "model (Tier 5) produces distributional rather than point forecasts, and the §3.7.4 "
    "robustness check replaces the parametric Gaussian safety-stock formula with an "
    "empirically calibrated quantile spread. This is a principled extension of "
    "classical inventory theory to non-Gaussian demand distributions. The simulation "
    "results presented in Chapter 5 show that this extension does not pay off "
    "operationally in the M5 test regime — but that finding itself is a contribution: "
    "it bounds the conditions under which distributional safety stock is worth "
    "adopting, providing both researchers and practitioners with a clearer expectation "
    "of when the more complex policy yields measurable benefit.")
add_para(doc,
    "On the responsible-AI dimension, model transparency is established through SHAP "
    "analysis of the best-performing model (top five features account for 78.6% of "
    "model behaviour). Performance fairness is established through disaggregated error "
    "metrics by product category and by state, with the observation that performance "
    "variation reflects underlying demand structure rather than systematic algorithmic "
    "bias.")
...
add_para(doc,
    ...
    "elsewhere. This diagnostic capability is, in the present author's view, more "
    "operationally valuable than the headline cost-reduction figures typically reported "
...
add_para(doc,
    ...
    "is exposed to genuine stockout risk. It addresses Gap 3 (SME accessibility) "
    "through tooling and infrastructure choices and by delivering the simulation "
    "engine as a pre-investment diagnostic. It partially closes Gap 2 (deployment) "
    "through the dashboard and CSV refresh architecture, and Gap 4 (uncertainty and "
    "responsible AI) through quantile forecasting, SHAP transparency, and fairness "
    "analysis. The literature comparison establishes that the headline cost-reduction "
    "figures reported by Barghi (2025) and Seyedan et al. (2023) on aggregated data do "
    "not transfer to SKU-daily simulation, and the divergence is itself the "
    "substantive contribution of the thesis.")
add_para(doc,
...
OUT = "/Users/desmond/Capstone Project/retail-demand-forecasting/Chapter_6_Discussion_v2.docx"
doc.save(OUT)
print(f"Saved: {OUT}")
